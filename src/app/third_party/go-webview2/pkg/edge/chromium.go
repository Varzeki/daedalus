...
const (
	hResultErrorInvalidState uint32 = 0x8007139f
	webView2InitRetryDelay          = 200 * time.Millisecond
	webView2InitMaxRetries          = 60
)
...
type Chromium struct {
	...
	initThread                uintptr
	focusOnInit               bool
	pendingCreateEnvironment  bool
	pendingCreateController   bool
	controllerCreationRetries int
...
func (e *Chromium) scheduleControllerCreateRetry() {
	e.pendingCreateController = true
	go func(thread uintptr) {
		time.Sleep(webView2InitRetryDelay)
		w32.User32PostThreadMessageW.Call(thread, w32.WMApp, 0, 0)
	}(e.initThread)
}

func (e *Chromium) scheduleEnvironmentCreateRetry() {
	e.pendingCreateEnvironment = true
	e.pendingCreateController = false
	go func(thread uintptr) {
		time.Sleep(webView2InitRetryDelay)
		w32.User32PostThreadMessageW.Call(thread, w32.WMApp, 0, 0)
	}(e.initThread)
...
func (e *Chromium) releaseEnvironmentObjects() {
	if e.webview != nil {
		e.webview.vtbl.Release.Call(uintptr(unsafe.Pointer(e.webview)))
		e.webview = nil
	}
	if e.controller != nil {
	...
	if e.environment != nil {
		e.environment.vtbl.Release.Call(uintptr(unsafe.Pointer(e.environment)))
		e.environment = nil
	}
}

func (e *Chromium) startEnvironmentCreation() bool {
	var userDataFolder *uint16
	if e.dataPath != "" {
		userDataFolder = windows.StringToUTF16Ptr(e.dataPath)
	}

	res, err := createCoreWebView2EnvironmentWithOptions(nil, userDataFolder, 0, e.envCompleted)
	if err != nil {
...
func (e *Chromium) requestControllerCreation() {
	if e.environment == nil {
		log.Printf("Creating controller failed because the environment is nil")
		atomic.StoreUintptr(&e.inited, 2)
		return
	}

	res, _, _ := e.environment.vtbl.CreateCoreWebView2Controller.Call(
		uintptr(unsafe.Pointer(e.environment)),
		e.hwnd,
		uintptr(unsafe.Pointer(e.controllerCompleted)),
	)
	if int32(uint32(res)) < 0 {
		if uint32(res) == hResultErrorInvalidState && e.controllerCreationRetries < webView2InitMaxRetries {
			e.controllerCreationRetries++
			log.Printf("CreateCoreWebView2Controller returned %08x; recreating environment (%d/%d)", uint32(res), e.controllerCreationRetries, webView2InitMaxRetries)
			e.scheduleEnvironmentCreateRetry()
			return
		}
...
func (e *Chromium) Embed(hwnd uintptr) bool {
	e.hwnd = hwnd
	e.initThread, _, _ = w32.Kernel32GetCurrentThreadID.Call()
	...
	for {
		...
		if msg.Message == w32.WMApp {
			if e.pendingCreateEnvironment {
				e.pendingCreateEnvironment = false
				e.releaseEnvironmentObjects()
				if !e.startEnvironmentCreation() {
					atomic.StoreUintptr(&e.inited, 2)
				}
				continue
			}
			if e.pendingCreateController && e.environment != nil {
				e.pendingCreateController = false
				e.requestControllerCreation()
				continue
			}
		}
...
func (e *Chromium) EnvironmentCompleted(res uintptr, env *ICoreWebView2Environment) uintptr {
	...
	if env == nil {
		log.Printf("Creating environment returned nil with %08x", uint32(res))
		atomic.StoreUintptr(&e.inited, 2)
		return 0
	}
...
func (e *Chromium) CreateCoreWebView2ControllerCompleted(res uintptr, controller *ICoreWebView2Controller) uintptr {
	if int32(uint32(res)) < 0 {
		if uint32(res) == hResultErrorInvalidState && e.controllerCreationRetries < webView2InitMaxRetries {
			e.controllerCreationRetries++
			log.Printf("CreateCoreWebView2Controller callback returned %08x; recreating environment (%d/%d)", uint32(res), e.controllerCreationRetries, webView2InitMaxRetries)
			e.scheduleEnvironmentCreateRetry()
			return 0
		}
		log.Printf("Creating controller failed with %08x", uint32(res))
	...
	if controller == nil {
		log.Printf("Creating controller returned nil with %08x", uint32(res))
		atomic.StoreUintptr(&e.inited, 2)
		return 0
	}
	e.controllerCreationRetries = 0
	...
	if e.webview == nil {
		log.Printf("Creating controller returned a nil CoreWebView2 instance")
		atomic.StoreUintptr(&e.inited, 2)
		return 0
	}
